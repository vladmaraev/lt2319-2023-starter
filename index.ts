...
const API_KEY = 'AIzaSyBkPXqbqN3TgJhA-a63BMDVja8tnPJojsk';
...
const listen =
  ...
  ({ context }) =>
    ...
    async function fetchfromYoutube(prompt: string, max_tokens: number) {
      ...
      myHeaders.append(
        "Authorization",
        "Bearer AIzaSyBkPXqbqN3TgJhA-a63BMDVja8tnPJojsk",
      ),
    ...
    async function fetchFromChatGPT(prompt: string, max_tokens: number) {
      ...
      myHeaders.append(
        "Authorization",
        "Bearer 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 ",
      ),
      myHeaders.append("Content-Type", "application/json");
      const raw = JSON.stringify({
        model: "gpt-3.5-turbo",
